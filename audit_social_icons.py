...
for filename in html_files:
    ...
    modified = False
    # Regex to find <ion-icon name="..." style="..."> or similar variations
    for name, svg in SVGS.items():
        pattern = rf'<ion-icon\s+name=["\']{name}["\'][^>]*>.*?</ion-icon>'
    ...
    if modified:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Verified & Replaced SVGs in {filename}")

# Fix "support.html" specifically if it missed any
for filename in html_files:
    with open(filename, 'r', encoding='utf-8') as f:
        content = f.read()
    if 'support.html' in content:
        content = content.replace('support.html', 'support-us.html')
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Hard-fixed support.html link in {filename}")

print("Social SVG Audit Complete.")
