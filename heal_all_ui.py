...
def process_file(filepath):
    ...
    # ISSUE 1: manifest "FrankPass Generator" -> "FrankPass App"
    if 'manifest.json' in filepath:
        content = content.replace('"name": "FrankPass Generator"', '"name": "FrankPass App"')
        
    # ISSUE 2: 'Read Guide' download arrow removal in index.html (remove ion-icon download-outline)
    if 'index.html' in filepath:
        # Before: <a href="guide.html" class="btn-primary" ...><ion-icon name="download-outline"></ion-icon> Read Guide &amp; Buy Ebook</a>
        content = content.replace('<ion-icon name="download-outline"></ion-icon> Read Guide &amp; Buy Ebook', 'Read Guide &amp; Buy Ebook')
        
    # ISSUE 3 & 4: support.html -> support-us.html (and other .html files if any)
    if filepath.endswith('.html') or filepath.endswith('.js'):
        content = content.replace('href="/support.html"', 'href="/support-us.html"')
        content = content.replace("href='support.html'", "href='support-us.html'")
        content = content.replace('href="support.html"', 'href="support-us.html"')

    # ISSUES 5 & 6: Missing social media SVGs + missing toggleFooter scripts in footer logic
    # First, let's fix the SVG inline icons for offline footer
    SVGS = {
    ...
    if filepath.endswith('.html'):
        for name, svg in SVGS.items():
            content = content.replace(f'<ion-icon name="{name}"></ion-icon>', svg)
            
        # Fix missing footer.js inclusion in various files
        missing_footer_js = [
            'guide.html',
            'faq.html',
            'legal.html'
        ...
        # If the file is one of the ones missing footer.js
        if any(f in filepath for f in missing_footer_js):
    ...
    if content != original_content:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Fixed operations for {filepath}")

for root, _, files in os.walk('.'):
    for fl in files:
        if fl.endswith('.html') or fl.endswith('.js') or fl.endswith('.json'):
