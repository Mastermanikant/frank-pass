...
// Do NOT redefine them here Ã¢â‚¬â€ they were removed to prevent desync bugs.
...
// ==========================================
const ANNOUNCEMENT = {
    id: "guide_popup_v1", // New ID to show this popup to new flow users
    show: false, // Turned off for production privacy     
    title: "Ã°Å¸Å’Â Select Guide Language",
    message: "Choose your preferred language to read how to use FrankPass.",
    cta: "Close"
};
...
// ==========================================
// Platform normalization logic moved to frankpass-utils.js
// ==========================================

function normalizePlatformDisplay() {
    if (!platformEl) return;
    const raw = platformEl.value.trim();
    if (!raw) { updatePlatformClearBtn(); return; }

    platformEl.value = FrankPassUtils.getNormalizedPlatform(raw);
    updatePlatformClearBtn();
}

function clearPlatformField() {
    if (!platformEl) return;
    platformEl.value = '';
    platformEl.focus();
    updatePlatformClearBtn();
...
function updatePlatformClearBtn() {
    const btn = document.getElementById('platform-clear-btn');
    const hint = document.getElementById('platform-hint');
    if (!platformEl) return;
    
    const val = platformEl.value;
    if (btn) btn.style.display = val.length > 0 ? 'inline-flex' : 'none';
    
    // Update platform hint based on current input
    if (hint) {
        if (val.length > 0) {
            hint.textContent = FrankPassUtils.getSeedHint(val);
...
// BUG-10 FIX: select() text on focus instead of clearing Ã¢â‚¬â€ prevents mobile jitter
...
// Platform: normalize display on blur + update clear button visibility on input
if (platformEl) {
    platformEl.addEventListener('blur', normalizePlatformDisplay);
    platformEl.addEventListener('input', updatePlatformClearBtn);
    platformEl.addEventListener('change', updatePlatformClearBtn); // catches datalist selection
}

// Variant Counter Slider â€” bidirectional sync
const variantSliderEl = document.getElementById('variant-slider');
if (variantEl && variantSliderEl) {
    variantSliderEl.addEventListener('input', () => {
        variantEl.value = variantSliderEl.value;
        const lbl = document.getElementById('variant-slider-label');
        if (lbl) lbl.textContent = variantSliderEl.value;
    });
    variantEl.addEventListener('change', () => {
        const val = parseInt(variantEl.value) || 1;
        const clamped = Math.min(val, parseInt(variantSliderEl.max));
        variantSliderEl.value = clamped;
        const lbl = document.getElementById('variant-slider-label');
        if (lbl) lbl.textContent = clamped;
    });
    variantEl.addEventListener('input', () => {
        const val = parseInt(variantEl.value) || 1;
        const clamped = Math.min(val, parseInt(variantSliderEl.max));
        variantSliderEl.value = clamped;
        const lbl = document.getElementById('variant-slider-label');
...
function autoDetectRegion() {
    try {
        ...
        if (detectedRegion) {
            // 1. Personalize Platform List Ã¢â‚¬â€ match "(XX)" at end of option value
...
if (regionEl) {
    regionEl.addEventListener('input', (e) => {
        ...
        // Update URL path removed to prevent 404s
    });
...
// Ensure country is loaded based on URL if accessed via /in etc.
window.addEventListener('load', () => {
    setTimeout(() => {
        const path = window.location.pathname.replace('/', '').toLowerCase();
        if (path && path.length === 2 && regionEl) {
            for(let i=0; i<regionEl.options.length; i++) {
                if (extractCountryCode(regionEl.options[i].value) === path) {
                    regionEl.selectedIndex = i;
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Unified platform normalization logic using shared utils
        const platform = FrankPassUtils.getNormalizedPlatform(platformEl.value);
...
function displayGeneratedPassword(password, profile, length) {
    ...
    if (strengthIndicator) {
        ...
        if (length >= 16 && profile === 'standard') {
            strengthIndicator.innerHTML = '<ion-icon name="lock-closed"></ion-icon> Unbreakable';
    ...
    switch (platform) {
        ...
        case 'instagram':
            navigator.clipboard.writeText(message).then(() => {
                alert("Message copied to clipboard! Now opening Instagram Ã¢â‚¬â€ paste it in your story or post.");
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (isStandalone) {
        // Running inside installed PWA Ã¢â‚¬â€ hide both banners & install btn
        const btn = document.getElementById('pwa-install-btn');
        if (btn) btn.style.display = 'none';
        hideInstallBanner();
    } else if (localStorage.getItem('frankpass-installed') === 'true') {
        // Previously installed Ã¢â‚¬â€ show Open in App options
        showOpenInAppBtn();
        showOpenInAppBanner();
    }
...
function checkNotifModal() {
    // Only show on root/index or 2-letter country paths
    const isRoot = window.location.pathname === '/' || window.location.pathname === '/index.html' || window.location.pathname === '';
    const isCountryPath = /^\/[a-zA-Z]{2}(\/)?$/.test(window.location.pathname);
    if (!isRoot && !isCountryPath) return;
...
window.addEventListener('load', () => {
    // Popups disabled for better SEO & UX
});
